...
class Symbolic_KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2):
        ...
        self.in_dim = in_dim
        self.mask = torch.nn.Parameter(torch.zeros(out_dim, in_dim)).requires_grad_(
            False
        )
        # torch
        self.funs = [
            [lambda x: x for i in range(self.in_dim)] for j in range(self.out_dim)
        ]
        # name
        ...
        if isinstance(fun_name, str):
            ...
            if x == None or y == None:
                ...
                params, r2 = fit_params(
                    x, y, fun, a_range=a_range, b_range=b_range, verbose=verbose
                )
                self.funs[j][i] = fun
